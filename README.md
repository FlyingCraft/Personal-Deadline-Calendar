...
管理员账号 `Re_Chan` 必须使用单独的一次性管理员邀请码注册。普通邀请码不能赋予管理员权限。管理员生成的邀请码一次使用后失效；明文只在生成时显示。移除成员会级联删除其个人日历，保留共享 Supabase 项目的其他应用账号。v1.3 将日子账号与旧日历的注册触发器隔离。

注册和登录使用用户名与密码，不收集邮箱。Supabase Auth 在服务端使用由用户名确定的内部账号标识；由于没有真实邮箱，当前版本不支持邮件找回密码，请妥善保存密码和日历迁移密钥。
